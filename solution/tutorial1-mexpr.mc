...
let pow : Float -> Int -> Float =
  lam x : Float. lam n : Int.
  -- we ensure tail recursion by using accumulators
    recursive let mul = lam acc. lam n.
      if eqi n 0 then acc
      else mul (mulf x acc) (subi n 1)
    in
    recursive let div = lam acc. lam n.
      if eqi n 0 then acc
      else div (divf acc x) (addi n 1)
    in
    if eqf x 0. then
      if gti n 0 then 0. else error "Undefined"
    else if eqi n 0 then 1.
    else if eqf x 0. then 0.
    else if eqi n 1 then x
    else
      if gti n 0 then mul 1. n else div 1. n
...
recursive let treeLookup = lam key. lam tree.
  switch tree
    case Leaf _ then None {}
    case Node (r & { data = (nodeKey, val) }) then
      if eqi key nodeKey then Some val
      else if lti key nodeKey then treeLookup key r.left
      else  treeLookup key r.right
  end
end
